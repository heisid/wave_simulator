...
int lineOscillatorX = SCREEN_WIDTH - 1;
int lineOscillatorYStart = 300;
int lineOscillatorYEnd = 500;
...
int main() {
  ...
  while (!WindowShouldClose()) {
    ...
    if (oscillatorOn) {
      ...
      for (int yPos = lineOscillatorYStart; yPos <= lineOscillatorYEnd;
           yPos++) {
        amplitudes[getFlatIndex(lineOscillatorX / FIELD_RESOLUTION,
                                yPos / FIELD_RESOLUTION)] = oscillatorAmplitude;
      }
    }
    updateVelocityField();
    updateAmplitudes();
